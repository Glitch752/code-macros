<script setup>
    ...
    let props = defineProps(["codeGroupData", "dragCode"]);
...
<template>
    <div class="codeGroup" :class="{margin: props.margin ?? true}">
        ...
        <template v-if="expanded">
            <div v-for="codeType in codeGroupData.code" :key="codeType.value">
                <div class="codeType" v-if="typeof codeType === 'string'" @mousedown="props.dragCode(codeType)">
                    <span class="name">{{codeTypeValue(codeType).name}}</span>
                    <span class="description">{{codeTypeValue(codeType).description}}</span>
                </div>
                <div v-else>
                    <CodeGroup :codeGroupData="codeType" :dragCode="props.dragCode" />
                </div>
            </div>
        </template>
    </div>
